...
using Judgy.Providers.DeepSeek.Tests.Fakes;
using Xunit;

namespace Judgy.Providers.DeepSeek.Tests;
...
public class DeepSeekProviderTests
{
    ...
    private static DeepSeekProviderOptions MakeOptions(FakeHttpMessageHandler handler) =>
        new()
        {
            ApiKey = ValidApiKey,
    ...
    public async Task CompleteAsync_SendsCorrectRequest()
    {
        ...
        using var provider = new DeepSeekProvider(MakeOptions(handler));
        ...
        handler.LastRequest.RequestUri!.PathAndQuery.Should().Be("/chat/completions");
    ...
    {
        var act = () => new DeepSeekProvider(null!);
    ...
    {
        var act = () => new DeepSeekProvider(new DeepSeekProviderOptions { ApiKey = "" });
    ...
    public void Options_DefaultValues()
    {
        ...
        options.Model.Should().Be("deepseek-chat");
    ...
    public async Task CompleteAsync_UsesCustomModel()
    {
        ...
        using var provider = new DeepSeekProvider(options);
        var request = new Judgy.Providers.LlmRequest(TestPrompt);

        await provider.CompleteAsync(request, TestContext.Current.CancellationToken);

        using var doc = JsonDocument.Parse(handler.LastRequestBody!);
        doc.RootElement.GetProperty("model").GetString().Should().Be("deepseek-reasoner");
    }
