...
using Judgy.Providers.Mistral.Tests.Fakes;
using Xunit;

namespace Judgy.Providers.Mistral.Tests;
...
public class MistralProviderTests
{
    ...
    private static MistralProviderOptions MakeOptions(FakeHttpMessageHandler handler) =>
        new()
        {
            ApiKey = ValidApiKey,
    ...
    public async Task CompleteAsync_SendsCorrectRequest()
    {
        ...
        using var provider = new MistralProvider(MakeOptions(handler));
    ...
    public async Task CompleteAsync_ParsesResponseCorrectly()
    {
        ...
        response.ProviderName.Should().Be("Mistral");
    ...
    {
        var act = () => new MistralProvider(null!);
    ...
    {
        var act = () => new MistralProvider(new MistralProviderOptions { ApiKey = "" });
    ...
    {
        var options = new MistralProviderOptions();

        options.Model.Should().Be("mistral-medium-latest");
    ...
    public async Task CompleteAsync_UsesCustomModel()
    {
        ...
        using var provider = new MistralProvider(options);
        var request = new Judgy.Providers.LlmRequest(TestPrompt);

        await provider.CompleteAsync(request, TestContext.Current.CancellationToken);

        using var doc = JsonDocument.Parse(handler.LastRequestBody!);
        doc.RootElement.GetProperty("model").GetString().Should().Be("mistral-small-latest");
    }
